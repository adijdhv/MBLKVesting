...
function TransferFixedReleasableAmount(_beneficiaryAddress,_index) public  onlyAdmin{

        bytes32 vestingScheduleId = getVestingScheduleByAddressAndIndex( _beneficiaryAddress,_index );
        ...
        require(isReleasor,"TokenVesting: only admins can release vested tokens");
        ...
        SafeTransferLib.safeTransfer(_token, beneficiaryPayable, releasableAmount); 
        FixedRewards -= releasableAmount; 
}

function TransferDynamicReleaseableAmount(_beneficiaryAddress,_amount) public onlyAdmin{
  bool isReleasor = (msg.sender == Admin);
        require( isReleasor, "TokenVesting: only Admins can release vested tokens");
...
function DepositTokensInDynamicRewardPool(uint256 _amount) public onlyOwner{
        SafeTransferLib.safeTransfer(_token, address(this), _amount);
        DynamicRewards += _amount;
}
